...
=======
      const faculties = await Faculty.find({ approved: true });
...
=======
    const { name, code, abbreviation, type, hours, weeklyHours, credit, semester } = req.body;
...
=======
    ...
    if (!semester || !name || !code || !abbreviation || !credit || !subjectHours || !type) {
      return res.json({
        success: false,
        message: "All fields are required"
      });
...
=======
    ...
    // Get all approved faculties
    const faculties = await Faculty.find({ approved: true }).lean();
    console.log(`Found ${faculties.length} approved faculties`);
...
=======
      return res.status(400).json({
        success: false,
        message: "Invalid Excel structure: header row not found"
      });
    }
...
<<<<<<< HEAD
    ...
    res.status(500).json({
      ...
      error: err.message
=======
      const rollNo = row.getCell(columnIndex.rollno).text.trim();
      const name = row.getCell(columnIndex.name).text.trim();
      const regNo = row.getCell(columnIndex.regno).text.trim();
      const mobile = row.getCell(columnIndex.mobile).text.trim();
      const email = row.getCell(columnIndex.email).text.trim();

      if (!regNo || !email.includes("@")) {
        skipped++;
        continue;
      }
    ...
    fs.writeFileSync(path.join(uploadDir, "students_batch.txt"), batchId);
...
app.post("/admin/remove-student-file", async (req, res) => {
  try {
    const batchFile = path.join(uploadDir, "students_batch.txt");
    if (!fs.existsSync(batchFile)) return res.json({ success: false, message: "No uploaded student batch found" });

    const batchId = fs.readFileSync(batchFile, "utf8").trim();
    const filePath = path.join(uploadDir, `students_${batchId}.xlsx`);

    // Delete only students from this batch
    await Student.deleteMany({ uploadedFromExcel: true, uploadBatchId: batchId });

    // Remove Excel file and batch file
    if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
    fs.unlinkSync(batchFile);

    res.json({ success: true, message: "File removed and students deleted successfully." });
...
=======
app.get("/admin/check-student-file", (req, res) => {
  const batchFile = path.join(uploadDir, "students_batch.txt");
  if (fs.existsSync(batchFile)) {
    const batchId = fs.readFileSync(batchFile, "utf8").trim();
    const filePath = path.join(uploadDir, `students_${batchId}.xlsx`);
    if (fs.existsSync(filePath)) return res.json({ exists: true, fileName: `students_${batchId}.xlsx` });
  }
...
app.get("/faculty/allocations", async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      allocations: []
=======
app.get("/admin/faculty-allocations", async (req, res) => {
  try {
    const allocations = await FacultyAllocation.aggregate([
      {
        $lookup: {
          from: "faculties",
          let: { facultyUsername: "$facultyUsername" },
          pipeline: [
            {
              $match: {
                $expr: {
      ...
      },
      {
        $group: {
          ...
          username: { $first: "$facultyUsername" },
    ...
    res.json({ 
      ...
      message: allocations.length > 0 ? "Allocations found" : "No allocations found"
    });

  } catch (err) {
    console.error("Faculty allocation fetch error:", err);
    res.status(500).json({ 
      success: false, 
      allocations: [],
...
app.post("/admin/strict-allocate", async (req, res) => {
  try {
    ...
    console.log("📋 Strategy: Designation > Experience > Strict Requirements > Alternating Theory/Lab");

    /* ===============================
       1️⃣ FETCH ALL REQUIRED DATA
       =============================== */

    // Get approved faculties only
    const faculties = await Faculty.find({ approved: true }).lean();
    ...
    console.log(`📝 Found ${submittedPreferences.length} submitted preferences`);

    // Get workload configuration
    const workloads = await WorkloadConfig.find({}).lean();
    console.log(`⚙️ Found ${workloads.length} workload configurations`);
...
=======
    // Create preference map with sorted preferences (priority 1 = highest priority)
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
    const prefMap = {};
...
=======
    ...
    console.log(`🔬 Lab/Project subjects: ${labSubjects.length}`);

    /* ===============================
       3️⃣ SORT FACULTIES BY SENIORITY
       =============================== */
    ...
    // Sort faculties by seniority (designation > experience)
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
...
=======
      ...
      // Higher designation first
      if (priorityA !== priorityB) {
        return priorityA - priorityB;
      }
      
      // More experience first within same designation
      return (b.experience || 0) - (a.experience || 0);
    });

    console.log("\n👥 STRICT FACULTY SENIORITY ORDER:");
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
...
=======
    /* ===============================
       4️⃣ INITIALIZE TRACKING STRUCTURES
       =============================== */
    ...
    subjects.forEach(subject => {
      ...
      subjectAllocationMap[subject.code] = {
        ...
        allocatedTo: [],
        allocationCount: 0,
        maxAllocations: isTheory ? 1 : 2,
        isTheory: isTheory
      };
    });

    // Initialize each faculty's load
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
...
=======
    ...
    // Get subject by code
    const getSubject = (code) => subjectMap[code];

    // Check if faculty can take subject with STRICT requirements
    const canAllocateSubject = (username, subject, isTheory) => {
      const load = facultyLoad[username];
      if (!load) return false;
      ...
      // 1. Check workload hour limit
      if (subjectHours > load.remainingHours) {
        return false;
      }
      
      // 2. Check if faculty already has this subject
      if (facultySubjectHistory[username].has(subjectCode)) {
        return false;
      }
      
      // 3. For theory subjects - check if already allocated
      if (isTheory) {
        if (subjectAllocationMap[subjectCode].allocated) {
          return false;
        }
      ...
      // 4. For lab subjects - check max allocation limit
      if (!isTheory) {
        if (subjectAllocationMap[subjectCode].allocationCount >= subjectAllocationMap[subjectCode].maxAllocations) {
          return false;
        }
      ...
      // 5. Check if we've tried this subject before for this faculty
      if (load.attemptedSubjects.has(subjectCode)) {
        return false;
      }
      
      // 6. Additional limits to prevent over-allocation of one type
      if (isTheory && load.theoryCount >= 4) { // Limit to max 4 theory subjects
        return false;
      }
      
      if (!isTheory && load.labCount >= 4) { // Limit to max 4 lab subjects
        return false;
      }
    ...
    // Get next theory subject for faculty
    const getNextTheory = (username, isUnderloadRemedy = false) => {
      const load = facultyLoad[username];
      const preferences = prefMap[username];
      
      // 1. Try from preferences first
      if (preferences && preferences.theory && load.theoryPrefIndex < preferences.theory.length) {
        for (let i = load.theoryPrefIndex; i < preferences.theory.length; i++) {
          const pref = preferences.theory[i];
          const subject = getSubject(pref.code);
...
=======
      ...
      // 2. If underload remedy, try any available theory
      if (isUnderloadRemedy) {
        const availableTheories = theorySubjects
          .filter(theory => {
            const allocationInfo = subjectAllocationMap[theory.code];
            return !allocationInfo.allocated && canAllocateSubject(username, theory, true);
          })
          .sort((a, b) => (a.weeklyHours || 0) - (b.weeklyHours || 0));
        
        if (availableTheories.length > 0) {
          return { 
            subject: availableTheories[0], 
            priority: 15, 
            source: "underload-remedy" 
          };
        }
      }
      
      // 3. Try any available theory
      const availableTheories = theorySubjects
        .filter(theory => {
          const allocationInfo = subjectAllocationMap[theory.code];
          return !allocationInfo.allocated && canAllocateSubject(username, theory, true);
    ...
    // Get next lab subject for faculty
    const getNextLab = (username) => {
      const load = facultyLoad[username];
      const preferences = prefMap[username];
      
      // 1. Try from preferences first
      if (preferences && preferences.lab && load.labPrefIndex < preferences.lab.length) {
        for (let i = load.labPrefIndex; i < preferences.lab.length; i++) {
          const pref = preferences.lab[i];
          const subject = getSubject(pref.code);
...
<<<<<<< HEAD
    ...
    const allocateSubject = (username, subject, isTheory, priority = 15, source = "available", forQuotaFill = false) => {
      ...
      const allocation = {
        ...
        subjectType: normalizeSubjectType(isTheory ? "Theory" : (subject.type || "Lab")),
        subjectAbbreviation: subject.abbreviation || subject.code,
=======
      
      // 2. Try any available lab
      const availableLabs = labSubjects
        .filter(lab => {
          const allocationInfo = subjectAllocationMap[lab.code];
          return allocationInfo.allocationCount < allocationInfo.maxAllocations && 
                 canAllocateSubject(username, lab, false);
        })
        .sort((a, b) => {
          const countA = subjectAllocationMap[a.code].allocationCount;
          const countB = subjectAllocationMap[b.code].allocationCount;
          
          // PRIORITY 1: Labs that already have 1 faculty (need 2nd)
          if (countA === 1 && countB !== 1) return -1; // a has 1, b doesn't
          if (countB === 1 && countA !== 1) return 1;  // b has 1, a doesn't
          
          // PRIORITY 2: Labs with fewer allocations
          if (countA !== countB) return countA - countB;
          
          // PRIORITY 3: Labs with fewer hours (easier to fit)
          return (a.weeklyHours || 0) - (b.weeklyHours || 0);
        });
      
      if (availableLabs.length > 0) {
        return { 
          subject: availableLabs[0], 
          priority: 15, 
          source: "available" 
        };
      }
    ...
    const allocateSubject = (username, subject, isTheory, priority = 15, source = "available") => {
      const load = facultyLoad[username];
      if (!load) return false;
      ...
      // Final check before allocation
      if (!canAllocateSubject(username, subject, isTheory)) {
        return false;
      }
      ...
      const allocation = {
        subjectCode: subjectCode,
        subjectName: subject.name,
        subjectType: isTheory ? "Theory" : (subject.type?.charAt(0).toUpperCase() + subject.type?.slice(1) || "Lab"),
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
...
=======
      ...
      if (isTheory) {
        ...
        subjectAllocationMap[subjectCode].allocated = true;
        subjectAllocationMap[subjectCode].allocatedTo.push(username);
        allocatedSubjects.add(subjectCode);
      } else {
        load.labCount++;
        subjectAllocationMap[subjectCode].allocationCount++;
        subjectAllocationMap[subjectCode].allocatedTo.push(username);
    ...
    // Check if faculty is underloaded (less than 50% of max hours)
    const checkUnderloaded = (load) => {
      const loadPercent = (load.usedHours / load.maxHours) * 100;
      return loadPercent < 50;
    };

    /* ===============================
       6️⃣ MAIN ALLOCATION LOOP - FIXED VERSION
       =============================== */
    ...
    // Process each faculty in strict seniority order
    for (const faculty of seniorityOrder) {
      const username = faculty.username.trim().toLowerCase();
      const load = facultyLoad[username];
      
      console.log(`\n🎯 ALLOCATING FOR: ${faculty.name} (${faculty.role}, ${faculty.experience} yrs)`);
      console.log(`  Seniority Rank: ${load.seniorityIndex + 1}/${seniorityOrder.length}`);
      console.log(`  Workload: ${load.maxHours}h max, ${load.minTheory} min theory, ${load.minLab} min lab`);
      console.log(`  Starting with ${load.remainingHours} available hours`);
      
      let iteration = 0;
      const maxIterations = 100; // Increased for more attempts
      let consecutiveFailures = 0;
      const maxConsecutiveFailures = 10; // Increased
      
      // Reset allocation type for this faculty
      load.nextAllocationType = "theory";
      
      // Continue while faculty has capacity and hasn't hit max attempts
      while (load.remainingHours > 0 && iteration < maxIterations) {
        iteration++;
        
        // Check if we should stop due to many failures
        if (consecutiveFailures >= maxConsecutiveFailures) {
          console.log(`    ⏹️ ${maxConsecutiveFailures} consecutive failures, stopping`);
          break;
        }
        
        // Check if underloaded (less than 50%)
        const currentLoadPercent = (load.usedHours / load.maxHours) * 100;
        const isUnderloaded = currentLoadPercent < 50;
        
        console.log(`  🔄 Iteration ${iteration}: ${load.remainingHours}h remaining | Load: ${load.usedHours}/${load.maxHours}h (${Math.round(currentLoadPercent)}%)`);
        
        // FIRST PRIORITY: Ensure minimum theory requirements
        if (load.theoryCount < load.minTheory) {
          console.log(`    ⚠️ Need theory (${load.theoryCount}/${load.minTheory})`);
          const nextTheory = getNextTheory(username, isUnderloaded);
          
          if (nextTheory) {
            if (allocateSubject(username, nextTheory.subject, true, nextTheory.priority, nextTheory.source)) {
              console.log(`    ✅ Allocated theory: ${nextTheory.subject.code} (${nextTheory.subject.weeklyHours}h)`);
              console.log(`       Remaining: ${load.remainingHours}h | Theory: ${load.theoryCount}/${load.minTheory} | Lab: ${load.labCount}/${load.minLab}`);
              load.nextAllocationType = "lab";
              consecutiveFailures = 0;
              continue; // Continue to next iteration
            } else {
              consecutiveFailures++;
              console.log(`    ❌ Failed to allocate theory: ${nextTheory.subject.code}`);
            }
          } else {
            console.log(`    ⚠️ No available theory subjects`);
            consecutiveFailures++;
          }
          continue;
        }
        
        // SECOND PRIORITY: Ensure minimum lab requirements
        if (load.labCount < load.minLab) {
          console.log(`    ⚠️ Need lab (${load.labCount}/${load.minLab})`);
          const nextLab = getNextLab(username);
          
          if (nextLab) {
            if (allocateSubject(username, nextLab.subject, false, nextLab.priority, nextLab.source)) {
              console.log(`    ✅ Allocated lab: ${nextLab.subject.code} (${nextLab.subject.weeklyHours}h)`);
              console.log(`       Remaining: ${load.remainingHours}h | Theory: ${load.theoryCount}/${load.minTheory} | Lab: ${load.labCount}/${load.minLab}`);
              load.nextAllocationType = "theory";
              consecutiveFailures = 0;
              continue;
              ...
              consecutiveFailures++;
              console.log(`    ❌ Failed to allocate lab: ${nextLab.subject.code}`);
            }
          } else {
            console.log(`    ⚠️ No available lab subjects`);
            consecutiveFailures++;
          }
          continue;
        }
        
        // THIRD PRIORITY: Fill remaining hours with alternating subjects
        console.log(`    🔄 Alternating (next: ${load.nextAllocationType})`);
        
        let allocatedThisIteration = false;
        
        if (load.nextAllocationType === "theory") {
          const nextTheory = getNextTheory(username, isUnderloaded);
          if (nextTheory && allocateSubject(username, nextTheory.subject, true, nextTheory.priority, nextTheory.source)) {
            console.log(`    ✅ Allocated theory: ${nextTheory.subject.code} (${nextTheory.subject.weeklyHours}h)`);
            load.nextAllocationType = "lab";
            allocatedThisIteration = true;
          } else {
            // Try lab if theory failed
            const nextLab = getNextLab(username);
            if (nextLab && allocateSubject(username, nextLab.subject, false, nextLab.priority, nextLab.source)) {
              console.log(`    ✅ Allocated lab: ${nextLab.subject.code} (${nextLab.subject.weeklyHours}h)`);
              load.nextAllocationType = "theory";
              allocatedThisIteration = true;
            }
        ...
        if (allocatedThisIteration) {
          consecutiveFailures = 0;
          console.log(`       Remaining: ${load.remainingHours}h | Total: ${load.usedHours}/${load.maxHours}h`);
        } else {
          consecutiveFailures++;
          console.log(`    ⚠️ Failed to allocate any subject this iteration`);
        }
        
        // If no hours remaining or very few hours left, break
        if (load.remainingHours <= 2) {
          console.log(`    ⏹️ Minimal hours remaining (${load.remainingHours}h), stopping`);
          break;
        }
      }
      
      // FINAL CHECK: Try to remedy severe underload
      const finalLoadPercent = (load.usedHours / load.maxHours) * 100;
      load.underloaded = finalLoadPercent < 50;
      
      if (load.underloaded && load.remainingHours > 0) {
        console.log(`  🔄 Severe underload detected (${Math.round(finalLoadPercent)}%) - attempting remedy...`);
        
        // Try to add more theory first
        const extraTheory = getNextTheory(username, true);
        if (extraTheory && allocateSubject(username, extraTheory.subject, true, extraTheory.priority, "underload-remedy")) {
          console.log(`    ✅ Added theory for underload: ${extraTheory.subject.code} (${extraTheory.subject.weeklyHours}h)`);
        }
        
        // Try to add lab if still underloaded
        if (load.underloaded && load.remainingHours > 0) {
          const extraLab = getNextLab(username);
          if (extraLab && allocateSubject(username, extraLab.subject, false, extraLab.priority, "underload-remedy")) {
            console.log(`    ✅ Added lab for underload: ${extraLab.subject.code} (${extraLab.subject.weeklyHours}h)`);
          }
        }
      }
      
      // Final status update
      load.underloaded = (load.usedHours / load.maxHours) * 100 < 50;
      load.overloaded = load.usedHours > load.maxHours;
      
      console.log(`\n  📊 ${faculty.name} ALLOCATION SUMMARY:`);
      console.log(`     Hours: ${load.usedHours}/${load.maxHours}h (${Math.round((load.usedHours/load.maxHours)*100)}%)`);
      console.log(`     Theory: ${load.theoryCount}/${load.minTheory} ${load.theoryCount >= load.minTheory ? '✅' : '❌'}`);
      console.log(`     Lab: ${load.labCount}/${load.minLab} ${load.labCount >= load.minLab ? '✅' : '❌'}`);
      console.log(`     Status: ${load.underloaded ? '⚠️ UNDERLOADED' : load.overloaded ? '❌ OVERLOADED' : '✅ ADEQUATE'}`);
      
      if (load.allocations.length > 0) {
        console.log(`     Allocated Subjects (${load.allocations.length}):`);
        load.allocations.forEach((alloc, idx) => {
          console.log(`       ${idx + 1}. ${alloc.subjectCode} (${alloc.subjectType}): ${alloc.weeklyHours}h [${alloc.source}]`);
        });
      } else {
        console.log(`     ❌ No subjects allocated`);
      }
    }

    /* ===============================
       7️⃣ POST-ALLOCATION ANALYSIS
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
       =============================== */
...
=======
    // CORRECTED: Identify unallocated subjects properly
    const unallocatedSubjects = subjects.filter(subject => {
      const allocationInfo = subjectAllocationMap[subject.code];
      const isTheory = subject.type?.toLowerCase() === "theory";
      
      if (isTheory) {
        // For theory subjects: not allocated if not marked as allocated
        return !allocationInfo.allocated;
      } else {
        // For lab subjects: not allocated if allocationCount < 2 (needs 2 faculties)
        return allocationInfo.allocationCount < 2;
      }
    });

    // Identify underloaded faculties (less than 50% capacity)
    const underloadedFaculties = [];
    const fullyAllocatedFaculties = [];
    
    for (const username in facultyLoad) {
      const load = facultyLoad[username];
      const loadPercent = Math.round((load.usedHours / load.maxHours) * 100);
...
<<<<<<< HEAD
            ...
            subjectType: normalizeSubjectType(a.subjectType),
            subjectAbbreviation: a.subjectAbbreviation || a.subjectCode,
...
=======
      ...
      if (load.remainingHours === 0) {
        fullyAllocatedFaculties.push(load.faculty.name);
      }
    }

    // Generate comprehensive report
    console.log(`\n📊 ALLOCATION STATISTICS:`);
    console.log(`   Total Faculties: ${seniorityOrder.length}`);
    console.log(`   Total Subjects: ${subjects.length}`);
    console.log(`   Allocated Subjects: ${subjects.length - unallocatedSubjects.length}/${subjects.length}`);
    console.log(`   Unallocated Subjects: ${unallocatedSubjects.length}`);
    console.log(`   Total Allocations: ${Object.values(facultyAllocations).reduce((sum, arr) => sum + arr.length, 0)}`);
    console.log(`   Underloaded Faculties: ${underloadedFaculties.length}`);
    console.log(`   Fully Allocated Faculties: ${fullyAllocatedFaculties.length}`);

    if (underloadedFaculties.length > 0) {
      console.log(`\n⚠️ UNDERLOADED FACULTIES (<50% capacity):`);
      underloadedFaculties.sort((a, b) => a.seniorityRank - b.seniorityRank).forEach(f => {
        console.log(`   ${f.faculty} (${f.designation}, Rank: ${f.seniorityRank}): ${f.usedHours}/${f.maxHours}h (${f.loadPercent}%)`);
        console.log(`       Theory: ${f.theoryCount}/${f.minTheory}, Lab: ${f.labCount}/${f.minLab}`);
        console.log(`       Available hours: ${f.remainingHours}h`);
      });
    }

    if (unallocatedSubjects.length > 0) {
      console.log(`\n📦 UNALLOCATED SUBJECTS:`);
      unallocatedSubjects.forEach((subject, idx) => {
        console.log(`   ${idx + 1}. ${subject.code} (${subject.name}):`);
        console.log(`       Type: ${subject.type}, Hours: ${subject.weeklyHours}h, Semester: ${subject.semester}`);
      });
    } else {
      console.log(`\n✅ ALL SUBJECTS ALLOCATED SUCCESSFULLY!`);
    }

    /* ===============================
       8️⃣ SAVE ALLOCATIONS TO DATABASE
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
       =============================== */
    ...
    for (const username in facultyAllocations) {
      ...
      for (const alloc of facultyAllocations[username]) {
        ...
        allocationRecords.push({
          ...
          subjectName: subject.name,
          subjectType: normalizeSubjectType(alloc.subjectType),
          subjectAbbreviation: subject.abbreviation || subject.code,
    ...
    for (const username in facultyAllocations) {
      ...
      for (const alloc of facultyAllocations[username]) {
        ...
        allocationRecords.push({
          ...
          subjectName: alloc.subjectName || subject?.name || alloc.subjectCode,
          subjectType: alloc.subjectType,
          subjectAbbreviation: subject?.abbreviation || alloc.subjectCode,
...
=======
    ...
    // Prepare final faculty results sorted by seniority
    const finalResults = [];
    seniorityOrder.forEach(faculty => {
      const username = faculty.username.trim().toLowerCase();
      const load = facultyLoad[username];
      ...
      if (load) {
        ...
        const theoryAllocations = load.allocations.filter(a => a.isTheory);
        const labAllocations = load.allocations.filter(a => !a.isTheory);
...
=======
          ...
          theory: `${load.theoryCount}/${load.minTheory}`,
          lab: `${load.labCount}/${load.minLab}`,
          underloaded: load.underloaded,
          overloaded: load.overloaded,
          status: load.underloaded ? "Underloaded" : 
                  load.overloaded ? "Overloaded" : 
                  loadPercent === 100 ? "Full" : "Balanced",
          theorySubjects: theoryAllocations.map(a => `${a.subjectCode} (${a.weeklyHours}h)`),
          labSubjects: labAllocations.map(a => `${a.subjectCode} (${a.weeklyHours}h)`),
          totalSubjects: load.allocations.length,
          remainingHours: load.remainingHours
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
        });
      }
...
=======
    res.json({
      success: true,
      message: `Allocated ${savedCount} subject allocations using strict seniority-based strategy`,
      summary: {
        totalFaculties: seniorityOrder.length,
        totalSubjects: subjects.length,
        allocatedSubjects: subjects.length - unallocatedSubjects.length,
        unallocatedSubjects: unallocatedSubjects.length,
        totalAllocations: savedCount,
        underloadedFaculties: underloadedFaculties.length,
        fullyAllocatedFaculties: fullyAllocatedFaculties.length,
        allocationStrategy: "Strict Seniority > Alternating"
      },
...
app.post("/admin/clear-allocations", async (req, res) => {
  try {
    await FacultyAllocation.deleteMany({});
    res.json({ success: true, message: "All allocations cleared" });
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
  } catch (err) {
...
<<<<<<< HEAD
    ...
    const formattedAllocations = allocations.map(allocation => ({
      ...
      subjectType: normalizeSubjectType(allocation.subjectType),
...
=======
      ...
      const allocations = await Allocation.find({ 
          facultyId: facultyId,
          status: 'allocated' 
      }).populate('subjectId', 'code name type weeklyHours semester');
      
      // Format the response
      const formattedAllocations = allocations.map(allocation => ({
          subjectCode: allocation.subjectId?.code,
          subjectName: allocation.subjectId?.name,
          subjectType: allocation.subjectId?.type,
          weeklyHours: allocation.weeklyHours || allocation.subjectId?.weeklyHours,
          semester: allocation.subjectId?.semester,
...
// Allocation model (if not already defined)
const AllocationSchema = new mongoose.Schema({
  ...
  allocatedBy: { type: String }, // admin username
...
=======
    ...
    const allocationsWithDetails = await Promise.all(
      allocations.map(async (alloc) => {
        const subject = await Subject.findOne({ code: alloc.subjectCode });
        return {
          subjectCode: alloc.subjectCode,
          subjectName: subject?.name || alloc.subjectName || alloc.subjectCode,
          subjectType: alloc.subjectType,
          ...
          semester: subject?.semester || alloc.semester || "N/A",
          credit: subject?.credit || 0,
          allocationDate: alloc.allocatedAt || alloc.createdAt
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
        };
      })
    );
    ...
    const placeAtomic = (group, day, time) => {
      ...
      group.subjects.forEach(sub => {
        ...
        gridSearchMap[slotKey].entries.push({
          ...
          subjectType: normalizeSubjectType(sub.subjectId.type),
          parallelGroupId: sub.parallelGroupId
        });
            ...
            if (slotStartMin < selEndMin && slotEndMin > selStartMin) {
              if (slot.entries) {
                slot.entries.forEach(e => {
                  if (e.facultyId) {
                    busyFacultyIds.add(e.facultyId.toString());
                  }
                });
              }
            }
    ...
    const slots = (timetableDoc.slots || []).map(slot => {
      const entries = (slot.entries && slot.entries.length > 0)
        ? slot.entries.map(e => ({
          ...
          subjectType: normalizeSubjectType(e.subjectType) // Consistency with database
        }))
        : [];
...
// (All previous routes like student login, admin faculty management, Excel uploads, timelines, etc. remain unchanged)
>>>>>>> c4096a7aad376612299f45f17cb617d6e992e233
